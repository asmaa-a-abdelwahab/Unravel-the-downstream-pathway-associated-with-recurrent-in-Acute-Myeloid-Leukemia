...
setwd('/home/dna/NU_Bioinformatics_Diploma/3. Integrative-course/Project/')


df <- read.table("Kegg_pathways.txt", sep='\t', header=TRUE, fill=TRUE)
...
df <- df[df$adj_pval < 0.35,] # or 17 for 20 gene     #dt[dt$fct %in% vc,]
df <- df[df$Category == 'PATHWAY',]
dim(df)
...
df2 <- read.delim("TAM-David-GoPlot/mrnas.degs.aml (2).csv", sep=',', header=TRUE) 
...
l1 <- subset(circ, term == 'Focal adhesion', c(genes,logFC))
l2 <- subset(circ, term == 'Dilated cardiomyopathy', c(genes,logFC))
l3 <- subset(circ, term == 'Cardiac muscle contraction', c(genes,logFC))
GOVenn(l1,l2,l3, label = c('Focal adhesion', 'Dilated cardiomyopathy', 'Cardiac muscle contraction'))
